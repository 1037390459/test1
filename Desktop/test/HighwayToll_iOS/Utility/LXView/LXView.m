...
//创建UIButton
+ (UIButton *)createButtonWithFrame:(CGRect)frame title:(NSString *)title imageName:(NSString*)imageName bgImageName:(NSString*)bgImageName radius:(CGFloat)radius target:(id)target action:(SEL)sel  color:(UIColor *)color
{
    ...
    //[button.layer addAnimation:[CAAnimation scale] forKey:@"shakeToShow"];
    return button;
}
//创建UIButton
...
+ (UISearchController *)createSearchDisplayControllerWithSearchBar:(UISearchBar *)searchBar contentsController:(UIViewController *)viewController delegate:(id)delegete
{
    //UISearchDisplayController 与UISearch搭配使用 可以实现搜索时的联动效果
    UISearchController *dvc = [[UISearchController alloc]initWithSearchResultsController:viewController];
    
    dvc.delegate = delegete;
    dvc.searchResultsUpdater = delegete;
    
    return dvc;
}

//创建UISearchDisplayController
+ (UISearchController *)createSearchDisplayControllerWithFrame:(CGRect)frame headerViewOnTableView:(UITableView *)tableView contentsController:(UIViewController *)viewController delegate:(id)delegete
{
    //创建UISearchBar
