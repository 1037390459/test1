...
{
    static LXBlueToothManager *sharedAccountManagerInstance = nil;
...
- (void)initializerBLEManager{
    self.bleManager = [[LXMBlueTooth alloc] init];
...
- (void)scanPeripheral {
    ...
    [self.bleManager _scanBLEPeripherals:ScanDuration];  // 6
}
...
#pragma mark 适配新版本通道
    if (tmpCharacter == _readCharacteristic) {
        ...
        for (NSInteger i =0; i < receiveData.length; i ++) {
            DLog(@"readCharacteristic---receiveDataBytes ====== %d",byte[i]);
            [string appendString:[NSString stringWithFormat:@"%hhu\n",byte[i]]];
        }
        DLog(@"-------%@--------",string);
        [self.deviceCallBack removeAllObjects];
        if (byte[2] == (receiveData.length - 6)) {
            // DLog(@"--获取数据的长度--%lu",(unsigned long)receiveData.length);
        ...
        Byte *byte = (Byte *)[receiveData bytes];
        for (NSInteger i =0; i < receiveData.length; i ++) {
            DLog(@"日志纸质writeCharacteristic---receiveDataBytes ====== %@",receiveData);
        }
        [self.deviceCallBack removeAllObjects];
        if (byte[2] == (receiveData.length - 6)) {
            if (receiveData.length >= 6) {
