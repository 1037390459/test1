//
//  LXPasswordRetrievalViewController.m
//  HighwayToll_iOS
//
...
#import "LXPasswordUpdateViewController.h" // 密码修改
#import "LXInterface+UserCenter.h"
@interface LXPasswordRetrievalViewController ()
...
@implementation LXPasswordRetrievalViewController
...
- (void)Verification{
    ...
    NSString *phoneText = ((UITextField *)[self.view viewWithTag:1]).text;
...
- (void)click:(UIButton *)sender{
    [self Verification];
    LXWeakSelf(self);
    [LXNetManager postWithParamDictionary:[LXInterface postUserCenterFindPwd:((UITextField *)[self.view viewWithTag:1]).text]
                                 finished:
        ^(id responseObj) {
...
- (void)loginClick{
    ...
    if (self.vCode.length == 0) {
        [LXMessage showErrorMessage:@"验证码不能为空"];
        return;
    }
    LXPasswordUpdateViewController *VC = [LXPasswordUpdateViewController new];
    VC.vCode = self.vCode;
    NSString *phoneText = ((UITextField *)[self.view viewWithTag:1]).text;
    VC.phoneStr =  phoneText;
